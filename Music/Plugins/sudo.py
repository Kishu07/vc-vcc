...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            await message.reply_text(f"✅ Added **{user.mention}** as a Super User for arc OwO")
    ...
    if added:
        await message.reply_text(f"✅ Added **{mention}** as a Super User for arc OwO")
        return os.execvp("python3", ["python3", "-m", "Music"])
    await edit_or_reply(message, text="❌ Something wrong happened, check logs.")  
    return    
...
async def userdel(_, message: Message):
    ...
    if user_id not in await get_sudoers():
        return await message.reply_text(f"❌ Not a part of arc's Sudo.")        
    removed = await remove_sudo(user_id)
    if removed:
        await message.reply_text(f"✅ Removed **{mention}** from arc's Sudo.")
        return os.execvp("python3", ["python3", "-m", "Music"])
    await message.reply_text(f"❌ Something wrong happened.")
...
async def sudoers_list(_, message: Message):
    sudoers = await get_sudoers()
    text = "**__Sudo Users List of arc Music:-__**\n\n"
